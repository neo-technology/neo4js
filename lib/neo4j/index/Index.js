...
_.extend(neo4j.index.Index.prototype,
    ...
    /** @lends neo4j.index.Index# */
    {
        ...
        index:function (item, key, value) {
            return this._index(item, key, false, value);
        },
        unique:function (item, key, value) {
            return this._index(item, key, true, value);
        },
        _index:function (item, key, unique, value) {
            ...
            return allPromises.then(function (values, fulfill, fail) {
                var url = values[0], key = values[1], value = values[2], urls = values[3];

                index.db.web.post(urls[index.getType()] + "/" + index.name + (unique ? "?unique" : ""),
                    { key:key, value:value, uri:url }, function (result) {
                        fulfill({success:url == result.self, node:index.db.node(result.self)});
                    }, fail);
            });
        },
